...
class Writ:
    def __init__(self, size=10000, year = 2016):
        ...
        # 服务器配置
        self.sever_ip = '127.0.0.1:27017/admin'
        self.user_name = parse.quote_plus("root")
        self.password = parse.quote_plus("111111")
...
# 关闭数据库连接
    ...
    def Get_Data(self):
        Data = pd.read_csv('Origin.csv', usecols=['_id','s1', 's7', 's9', 's31', 's22', 's17', 's23', 's25', 's26', 's27', 's28', 'qwContent'])
        null_count = Data['qwContent'].isnull().sum()
        if null_count == 0:
            Pure_Data = Data.copy()
        else:
        ...
        self.Original_Data, self.Length = Pure_Data, self.size - null_count
        self.Datatable = self.New_Table(self.Length)
